...
def get_schedule(url, start_date, days=1):
    ...
    for day in range(days):
        ...
        for event in cal.walk("vevent"):
            ...
            if start.date() == current_date:
                ...
                daily_events.append((time_range, summary))
        events.append((current_date, daily_events))
...
async def daily_schedule():
    channel_id = 1098570053735174155  # Replace with your channel ID
    ...
    events = get_schedule(
        "https://cloud.timeedit.net/nackademin/web/1/ri65E5ZQ1Q48eCQZ05Q7dtDQZ58ZZ5D9BuAYZ6Qy875tAnFb0FCQ74DF35585AD8B23AA2D4.ics",
        today)

    if events:
        message = f"Dagens schema ({today.strftime('%Y-%m-%d')} - {today.strftime('%A')}):\n"
        for time_range, summary in events[0][1]:
...
async def veckoschema(ctx):
    ...
    message = "Veckans schema:\n"
    for date, daily_events in events:
        if daily_events:
            message += f'{42*"-"}\n'
            message += f"{date.strftime('%Y-%m-%d')} - {date.strftime('%A')}:\n"
            for time_range, summary in daily_events:
                message += f"{time_range}: {summary}\n"
